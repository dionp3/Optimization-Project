...
class PSO:
    ...
    def animate(self, i, ax):
        if self.first_iteration:
            ...
            print("f(x) sebelum =", [round(f(val), 3) for val in self.x], "\n")
            ...
            print(f"f(gBest) = {round(f(self.gBest), 3)}")
            print(f"f(x) = {[round(f(x), 3) for x in self.x]}")
            print()
...
print("Bilangan acak untuk nilai awal X0:", initial_x_rounded)
